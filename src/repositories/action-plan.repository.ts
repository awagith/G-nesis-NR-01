...
export class ActionPlanRepository extends BaseRepository<ActionPlan> {
  ...
  async findByStatus(organizationId: string, status: ActionPlan['status']): Promise<QueryListResult<ActionPlan>> {
    const { data, error, count } = await db
      .from('action_plans').select('*', { count: 'exact' })
  ...
  async createItem(payload: Omit<ActionItem, 'id' | 'created_at' | 'completed_at'>): Promise<QueryResult<ActionItem>> {
    const { data, error } = await db
      .from('action_items').insert(payload).select().single()
    return { data: data as ActionItem | null, error: formatError(error) }
  }

  async updateItem(id: string, payload: Partial<Omit<ActionItem, 'id' | 'created_at'>>): Promise<QueryResult<ActionItem>> {
    const updates = {
      ...payload,

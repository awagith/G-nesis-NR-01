...
export class OrganizationRepository extends BaseRepository<Organization> {
  ...
  async createUnit(payload: Omit<OrganizationUnit, 'id' | 'created_at'>): Promise<QueryResult<OrganizationUnit>> {
  ...
  async updateUnit(id: string, payload: Partial<Omit<OrganizationUnit, 'id' | 'created_at'>>): Promise<QueryResult<OrganizationUnit>> {
    const { data, error } = await db
      .from('organization_units')
      .update(payload)
      .eq('id', id)
